...
async function openMessage(msgId) {
    try {
        ...
        let scanHtml = '';
        if (msg.scan) {
            const date = new Date(msg.scan.created_at).toLocaleString('de-DE');
            let geoHtml = '📍 IP-based';
            if (msg.scan.latitude && msg.scan.longitude) {
                const lat = msg.scan.latitude.toFixed(4);
                const lon = msg.scan.longitude.toFixed(4);
                const mapsUrl = `https://maps.google.com/?q=${lat},${lon}`;
                geoHtml = `📍 <a href="${mapsUrl}" target="_blank" style="color: #3b82f6; text-decoration: underline; cursor: pointer;">${lat}, ${lon}</a>`;
            }

            scanHtml = `<div style="margin-top: 20px; padding-top: 20px; border-top: 1px solid #ddd;">
                <h4 style="margin-top: 0; margin-bottom: 12px;">📍 Scan-Standort</h4>
                <div style="padding: 12px; background: #f9f9f9; border-radius: 6px; font-size: 13px; border-left: 3px solid #667eea;">
                    <div style="font-weight: 500;">${msg.scan.device_type} • ${msg.scan.browser_name}</div>
                    <div style="color: #666; font-size: 12px; margin: 4px 0;">${msg.scan.country || 'Unknown'}${msg.scan.city ? ', ' + msg.scan.city : ''}</div>
                    <div style="margin: 4px 0;">${geoHtml}</div>
                    <div style="color: #999; font-size: 11px;">Gescannt: ${date}</div>
                </div>
            </div>`;
        }
